...
export default function ShoppingCartSheet({ cart, updateQuantity, removeFromCart }) {
  ...
  const handleWhatsAppCheckout = () => {
    ...
    const phoneNumber = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || 'YOUR_PHONE_NUMBER'
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <Link href="/" className="mt-4 inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-full">
                Browse products
              </Link>
            </div>
          ) : (
                ...
                <div key={item.id} className="flex gap-4 pb-4 border-b last:border-b-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <button className="text-gray-400 hover:text-gray-600" aria-label={`Edit ${item.name}`}>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center border border-gray-300 rounded-md">
                        <button
                          ...
                          className={`px-3 py-1 hover:bg-gray-50 transition-colors ${item.quantity <= 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="px-4 py-1 text-sm font-medium border-x border-gray-300" aria-live="polite">
                          {item.quantity}
                        </span>
                        <button
        ...
        {cart.length > 0 && (
          <div className="border-t bg-white px-6 py-4">
            ...
            <button
              ...
              className="mt-3 w-full bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-full py-3 flex items-center justify-center gap-2 transition-all shadow-md"
